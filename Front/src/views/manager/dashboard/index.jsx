...
const DashDefault = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const summaryRes = await axios.get('http://localhost:5231/api/manager/dashboard/summary');
        setSummary(summaryRes.data);

        const monthlyRes = await axios.get('http://localhost:5231/api/manager/dashboard/user-registrations/monthly');
