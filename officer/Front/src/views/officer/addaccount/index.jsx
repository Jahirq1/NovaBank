...
const RegisterForm = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:5231/api/search/look?searchTerm=${searchTerm}`);
        if (response.ok) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('A je i sigurt që dëshiron të fshish këtë përdorues?')) {
      try {
        const response = await fetch(`http://localhost:5231/api/search/delete/${id}`, {
          method: 'DELETE',
        });
        if (response.ok) {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5231/api/search/update/${editFormData.id}`, editFormData, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://localhost:5231/api/auth/create', userToSend);
