...
const CreditApplicationForm = () => {
  ...
  const fetchLoans = async () => {
    try {
      const response = await axios.get('http://localhost:5231/api/loans/status');
  ...
  const handleViewLoanPdf = async (loanId) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5231/api/loans/pdf/${loanId}`, {
        responseType: 'blob',
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://localhost:5231/api/loans/create', loanDto);
